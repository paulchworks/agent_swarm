...
Enter Task → click Run Swarm

UI calls POST /api/run/start → saves run_id

UI opens EventSource(/api/stream/{run_id}) and renders Live Trace

On done, UI fetches /api/result/{run_id} and fills Status & Output
...
Quick single-host example; adapt to your registry and base images.
...
Open the UI → enter a Task.

Click Run Swarm.

Watch Live Trace fill with logs/hand-offs.

When done, see Status & Output with final result and node history.
