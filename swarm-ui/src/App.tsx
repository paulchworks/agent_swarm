...
const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ className, ...rest }) => (
  <button
    {...rest}
    className={`px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 font-semibold disabled:opacity-60 ${className ?? ''}`}
  />
)
...
}: { tabs: string[]; active: string; onChange: (key: string) => void }) {
  return (
...
export default function App() {
  ...
  const runSwarm = async () => {
    ...
    setError(null); setResp(null); setRunId(null); clearTraces()
    ...
    try {
      ...
      const startRes = await fetch(buildUrl('api/run/start'), {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload),
      })
      if (!startRes.ok) throw new Error(`HTTP ${startRes.status}`)
      const { run_id } = await startRes.json(); setRunId(run_id)

      const es = new EventSource(buildUrl(`api/stream/${run_id}`))
      esRef.current = es

      es.addEventListener('ready', () => addTrace({ type: 'ready', message: 'stream ready', run_id }))
      es.addEventListener('start', (e: MessageEvent) => {
        const data = safeParse(e.data); addTrace({ type: 'start', run_id, task: data?.task })
      })
      es.addEventListener('log', (e: MessageEvent) => {
        const data = safeParse(e.data); addTrace({ type: 'log', level: data?.level || 'LOG', message: data?.message })
      })
      es.addEventListener('error', (e: MessageEvent) => {
        const data = safeParse((e as any).data); addTrace({ type: 'error', message: data?.error || 'server error' })
      })
      es.addEventListener('done', async (e: MessageEvent) => {
        const data = safeParse(e.data)
        addTrace({
          type: 'done', status: data?.status, has_output: data?.has_output,
          output_preview: data?.output_preview, transcript_preview: data?.transcript_preview,
        })
      ...
      es.addEventListener('summary', (e: MessageEvent) => {
        const data = safeParse(e.data)
        addTrace({
          type: 'summary', status: data?.status, has_output: data?.has_output,
          output_preview: data?.output_preview, transcript_preview: data?.transcript_preview,
        })
      })
      es.onerror = () => addTrace({ type: 'client-error', message: 'stream connection error' })
    } catch (e: any) {
      setError(e?.message || 'Request failed'); setLoading(false); setStreaming(false)
      esRef.current?.close(); esRef.current = null
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 text-slate-100">
      ...
      {/* Main: Console dominates, Config is compact on the right */}
      <main className="max-w-6xl mx-auto p-6 grid lg:grid-cols-3 gap-6">
        {/* LEFT: Command bar + Console */}
        <div className="lg:col-span-2 space-y-6">
          {/* Command bar (compact) */}
          <Card className="sticky top-4 z-10 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.6)]" right={
            <div className="flex items-center gap-2">
              {runId && <span className="hidden sm:block text-[11px] opacity-70">Run: <code className="bg-slate-800/70 px-1 py-0.5 rounded">{runId.slice(0,8)}…</code></span>}
              <Button onClick={runSwarm} disabled={loading || streaming}>
                {loading || streaming ? 'Running…' : 'Run'}
              </Button>
            </div>
          }>
            <Label>Task</Label>
          ...
          {/* Console with tabs (center stage) */}
          <Card title="Console">
            <Tabs tabs={['Trace', 'Transcript', 'Output']} active={activeTab} onChange={(t) => setActiveTab(t as any)} />

            {/* TRACE */}
            {activeTab === 'Trace' && (
              <div className="bg-slate-950 border border-white/10 rounded-xl p-3 text-xs font-mono max-h-[60vh] overflow-auto whitespace-pre-wrap break-words">
            ...
            <Accordion title="Entry & Limits" defaultOpen>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <select
                    className="w-full bg-slate-950 border border-white/10 rounded-lg px-2 py-1.5 text-sm"
                ...
                {agents.map((a, idx) => (
                  <div key={idx} className="rounded-lg bg-slate-950/60 border border-white/10 p-3">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        <Label>Model</Label>
                        <Input placeholder="optional" value={a.model || ''} onChange={(e) => updateAgent(idx, { model: e.target.value || undefined })} />
                      </div>
                    </div>
                    <div className="mt-2">
